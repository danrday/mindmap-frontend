...
class SplitterLayout extends React.Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        let equalProps = isEqual(this.props, nextProps)
        let equalState = isEqual(this.state, nextState)
    ...
    render() {
        ...
        for (let i = 0; i < children.length; ++i) {
            ...
            if (children.length > 1 && i !== primaryIndex) {
                ...
                // MONKEYPATCH
                if (this.props.navIsHovered || this.props.navIsOpen) {
                    // this is for if the navbar is closed and it gets resized, then opened again, we want to reset it from 0
                    size = this.state.secondaryPaneSize || this.props.secondaryInitialSize
                } else {
                    size = 0
                }
            }
